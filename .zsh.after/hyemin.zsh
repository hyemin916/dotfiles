...
export PATH="$PATH:/Users/hyemin/icloud/bin"

export PATH=":$PATH:/Users/hyemin/bin/ijhttp/"
...
export OH_MY_ZSH=/Users/hyemin/.oh-my-zsh/
...
# ---- Brew wrapper (auto-sync Brewfile) ----
brew() {
  command brew "$@" || return $?
  ...
  local type=$( $is_cask && echo "cask" || echo "brew" )
  ...
  case "$subcmd" in
    ...
    uninstall|remove)
      for pkg in "${pkgs[@]}"; do
        if grep -q "^${type} \"${pkg}\"" "$brewfile"; then
          sed -i '' "/^${type} \"${pkg}\"/d" "$brewfile"
          echo "Brewfile: removed ${type} \"${pkg}\""
        fi
      done
      ;;
  esac
